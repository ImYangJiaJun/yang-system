...
- 后端：Rust 2021（`rust-version = "1.80"`；仓库没有 `rust-toolchain` 文件，1.97.1 工具链由 `scripts/setup_local.ps1` 在本地安装、由 CI 的 `RUST_VERSION` 环境变量固定），axum（经 `yang-base` 的 `transport-axum`）、tokio、sqlx (MySQL)、Redis、jsonwebtoken、argon2（框架侧密码哈希）、lettre (SMTP)、tracing、metrics。
- 前端：React 19 / Vite 8 / Tailwind CSS 4 / shadcn/ui（组件源码入库 `frontend/src/shared/ui/`）/ TanStack Query + Table / react-router 7 / zod / Ajv（动态 JSON Schema 校验）/ react-hook-form，TypeScript ~5.9，pnpm 10.33.1（`frontend/package.json` 的 `packageManager` 字段固定），Node.js 24+。技术选型决策见 `docs/architecture/frontend-rebuild/` ADR 组。浏览器契约：Chrome/Edge ≥ 111、Firefox ≥ 128、Safari ≥ 16.4。
- 依赖服务：MySQL 8.0（`127.0.0.1:3306`）与 Redis 7（`127.0.0.1:6379`），由根目录 `compose.yaml` 提供；`docker/mysql/init/001-create-databases.sql` 会同时创建 `yang_system` 和 `yang_system_test` 两个库（utf8mb4）。注意 `docker compose down -v` 会永久删除本地数据卷。
- 辅助脚本：Python 3.11+（质量门禁、架构检查、Action 脚手架、本地初始化、OpenAPI 导出）。
...
tests/                       # Rust 集成测试（真实 MySQL/Redis）与 benchmark；共享 helper 在 tests/common/
examples/                    # frontend_demo/（无数据库演示后端，供 Playwright 用）、openapi-dump.rs
scripts/                     # run_ci.py / check_architecture.py / new_action.py / dump_openapi.py / setup_local.ps1 / upgrade_local_config.py
docs/                        # contracts/（系统契约）、operations/（运维）、guides/（开发指南）、architecture/（决策/边界/路线/benchmarks）、assessments/（时点评估）
...
- `python scripts/run_ci.py full` — 推送前：在 quick 之上增加 `cargo test --all-targets --locked`、`cargo clippy --all-targets --all-features --locked -- -D warnings`、前端 `pnpm audit --prod --audit-level moderate`、前端完整 `pnpm check`，以及端口隔离的 dev-server（前端 5310 / 后端 18310）和 production-build（前端 5311 / 后端 18311）两套 Playwright。
...
- `python scripts/check_architecture.py` — 架构门禁：保证 `actions/` 中一个文件只承载一个 Action（恰好一个 `pub(super) async fn handle` + 一个自包含 `pub(super) fn register`）、文件与 `mod.rs` 的 `ACTIONS` 注册表数组一致，裸 SQL 路径有 `// tenant-boundary: <kind> <id>` 边界注释；前端侧还锁定 `main.tsx` 唯一 createRoot、`TableView.tsx` 行数与 hook 边界、`registry.ts` 静态字面量 import。
...
- `python scripts/dump_openapi.py`（前端侧别名 `pnpm gen:contracts`）— 导出后端 OpenAPI 3.1 契约快照（`frontend/contracts/openapi.json`）并再生成 TypeScript 类型（`frontend/src/engine/contracts/api-types.ts`）；两个生成物禁止手改，后端 Action/输入输出契约变更后必须重跑并提交。
- `pnpm --dir frontend check` — 前端完整检查链，依次为 `format:check`（Prettier）→ `lint`（ESLint `--max-warnings 0`）→ `typecheck`（`tsc --noEmit`）→ `test`（Vitest）→ `verify:locale-contract` → `build` → `verify:production-build` → `verify:bundle-budget` → `verify:deployment-contract`；单独排查可用同名子命令及 `e2e` / `e2e:production`。
- 首次环境初始化（Windows PowerShell 7，在 `lib_yang` 仓库根目录执行）：`pwsh -NoProfile -File project/yang-system/scripts/setup_local.ps1`，会启动 Compose 依赖、安装固定工具链与前端依赖并生成被 Git 忽略的 `config.toml`；`-CheckOnly` 只检查工具，`-UpgradeLegacyConfig` 迁移旧配置（先备份到 `target/local-config-backups/`）。
...
- Addon 层目录只承载 `mod.rs`（装配+对外端口）、module 子目录与 `domain/`（addon 级共享机制；业务用例流程内联在各 Action 文件，`domain/` 只承载模块上下文、持久化边界、授权声明、安全版本原语、状态机、邮件投递等共享机制）；module 层只承载 `mod.rs`（装配+展示投影）、`table.rs`（表声明）和 `actions/`。通用机制（密码哈希、限流、邮箱验证码、浏览器会话 Cookie）由 `yang_base::action::auth` 框架能力提供，应用侧不重复实现。
...
- `config.toml` 被 Git 忽略且**禁止提交**；仓库只保留无真实凭据的 `config.example.toml`（`config.show.toml` 为展示用样例）。配置按 `config.toml < YANG_SYSTEM_* 环境变量 < secret 目录` 合成，契约见 `docs/contracts/CONFIGURATION.md`。
...
- 生产镜像：后端 `docker/app/Dockerfile`（构建上下文必须是 lib_yang 仓库根：`docker build -f project/yang-system/docker/app/Dockerfile -t yang-system:local .`，配套根目录 `.dockerignore`；工具链与 CI 同为 1.97.1，debian-slim 运行时 + 非 root 用户）；前端 `frontend/deploy/Dockerfile`（构建上下文为 `frontend/`，配套该目录的 `.dockerignore`——缺了它 `COPY . .` 会把宿主机 `node_modules` 覆盖进容器；corepack 按 `packageManager` 固定 pnpm，运行时复用 CI 校验过的同一 Nginx 镜像与 `nginx.conf`）。前端 nginx 必须与后端共享网络命名空间（同 Pod / `--network container:`）运行——upstream 写死 `127.0.0.1:8080`。**2026-09-22 起 nginx 监听 `8081`（所有网卡），不再是 `127.0.0.1:8081`**：`docker -p` 的 DNAT 到不了 netns 的 loopback，两者技术上互斥；「不暴露公网」改为由编排层的 `-p ${BIND_ADDR}:<port>:8081` 承担（`BIND_ADDR` 默认 `127.0.0.1`，运维可显式覆盖为 `0.0.0.0`），并由 `frontend/scripts/verify-deployment-contract.mjs` 机械校验**默认值是 loopback**——注意该门禁**不检查运行时环境变量**，所以它不构成对公网暴露面的保证（详见 `deploy/README.md`）。
- 服务器部署：产物在 `deploy/`（本机 `deploy.ps1` 打包上传 → 服务器 `deploy-blue-green.sh` 蓝绿），部署目录 `/home/yjj/yang-system`，应用边缘宿主端口 **18654**（该服务器安全组只放行 `80/443` 与 `18000-19000` 段，换到段外会公网不可达；`LIVE_HOST_PORT` 可覆盖），绑定地址默认 loopback、本测试环境经 `deploy.ps1 -EdgeBindAddr 0.0.0.0` 显式暴露到公网（**明文 http**）；管理面宿主端口 9154 由 `METRICS_BIND_ADDR` 单独控制、**不随边缘暴露**。MySQL/Redis 由 `deploy/compose.infra.yaml` 起，**应用容器不要交给 compose 管理**。另有两个运维开关：`deploy.ps1 -SyncConfig`（推本机 `deploy/config.cloud.toml` 到服务器、就地热替换并重启应用；远端 `refresh` 会先备份、失败自动回滚）与 `-ResetDb`（`DROP DATABASE` + 重建 `[mysql].url` 指向的库 + 清 Redis，由应用启动时重建 schema；**不可逆**，两步确认：本机输入 yes / `-Yes`，远端 `RESET_DB_CONFIRM=yes`）。两者必须重启应用容器才生效：配置是单文件 bind mount，`mv`/`sed -i` 这类换 inode 的写法运行中的容器读不到新内容。清库后**没有任何账号能登录**（新账号零权限，授权只有手工 SQL，见 `docs/contracts/AUTHZ_GRANTS.md`）；旧 JWT 多数会因「用户不存在」被拒，但 id 从 1 重排可能让个别旧 Token 被新账号继承，要彻底作废须轮换 `[token].active_secret`。清空目标是本脚本管理的容器，`[mysql].url`/`[redis].url` 的主机名不是它时一律拒绝（没有逃生门）。
