...
- 生产镜像：后端 `docker/app/Dockerfile`（构建上下文必须是 lib_yang 仓库根：`docker build -f project/yang-system/docker/app/Dockerfile -t yang-system:local .`，配套根目录 `.dockerignore`；工具链与 CI 同为 1.97.1，debian-slim 运行时 + 非 root 用户）；前端 `frontend/deploy/Dockerfile`（构建上下文为 `frontend/`，配套该目录的 `.dockerignore`——缺了它 `COPY . .` 会把宿主机 `node_modules` 覆盖进容器；corepack 按 `packageManager` 固定 pnpm，运行时复用 CI 校验过的同一 Nginx 镜像与 `nginx.conf`）。前端 nginx 必须与后端共享网络命名空间（同 Pod / `--network container:`）运行——upstream 写死 `127.0.0.1:8080`。**2026-09-22 起 nginx 监听 `8081`（所有网卡），不再是 `127.0.0.1:8081`**：`docker -p` 的 DNAT 到不了 netns 的 loopback，两者技术上互斥；「不暴露公网」改为由编排层的 `-p 127.0.0.1:<port>:8081` 承担，并由 `frontend/scripts/verify-deployment-contract.mjs` 机械校验（详见 `deploy/README.md`）。
- 服务器部署：产物在 `deploy/`（本机 `deploy.ps1` 打包上传 → 服务器 `deploy-blue-green.sh` 蓝绿），部署目录 `/home/yjj/yang-system`，应用边缘宿主端口 **8154（仅绑 loopback）**；MySQL/Redis 由 `deploy/compose.infra.yaml` 起，**应用容器不要交给 compose 管理**。
