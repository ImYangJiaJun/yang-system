...
|---|---|---|
| `account.user` | 个人账户 / 用户中心 / 账号设置 | 邮箱验证码注册、登录（用户名/邮箱）、头像设置与展示、Refresh Cookie、全设备退出、当前用户、Step-up、账号中心（资料/改密/改用户名/换邮箱/停用/登录设备/安全事件）、自助密码找回；启用凭据版本切换后还提供改密、自助停用、密码重置、匿名化删除；管理动作（grants 化）：停用/启用用户、签发重置凭证、用户列表 |
