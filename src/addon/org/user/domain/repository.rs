...
use super::super::{ACTIVE_STATUS, IS_ADMIN, ORG_ID, STATUS, USER_ID};
use crate::addon::account::{increment_locked_authz_versions, lock_user_authorizations};
use crate::addon::org::organization::ACTIVE_STATUS as ACTIVE_ORG_STATUS;
use crate::audit;
...
pub(in crate::addon::org::user) async fn add(
    ctx: &ActionContext,
    input: Record,
) -> Result<InsertResult, BaseError> {
    let org_id = add_org_id(ctx, &input)?;
    let user_id = input.require::<i64>(USER_ID)?;
    let status = proposed_string(&input, STATUS, ACTIVE_STATUS)?;
...
pub(in crate::addon::org::user) async fn put(
    ctx: &ActionContext,
    input: PutInput,
...
pub(in crate::addon::org::user) async fn delete(
    ctx: &ActionContext,
    input: GetByPk,
) -> Result<AffectedResult, BaseError> {
    let id = primary_key(&input.id)?;
    ...
    let result = async {
        ...
        if membership.status == ACTIVE_STATUS && membership.admin {
            ensure_not_last_active_org_admin(&active_admins, id)?;
        }
        let locked =
...
fn removes_active_admin(
    ...
    membership.status == ACTIVE_STATUS
        && membership.admin
        && (change.next_status != ACTIVE_STATUS || !change.next_admin)
}
...
mod tests {
    use super::*;
    use crate::addon::org::user::ACTIVE_STATUS;
