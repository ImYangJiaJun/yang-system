...
- [x] **V4 前置**：动工**之前**先跑一次真实飞书审批提交抓 `linkage_params` 报文
      （设计文档 §11.2）。**2026-09-24 于云服务器抓到**，报文与预期**不符**：
      命中 `{"手动填写内容":"成都"}`——键是 `linkageConfigs[].key` 自由文本，
      **值是被联动控件的文案而不是 `@i18n@<option_id>`**。
      据此已改读端（同时认 `option_id` 与 `label`，同名文案取子树并集），
      详情与端到端证据见设计文档 §11.2 第 2 条。
