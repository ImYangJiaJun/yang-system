...
| P7 | 演示 addon 验证"新业务全流程接入"（验证审计结论"新增成本为零"） | 验证 | ✅ |

## 关键决策（ADR 摘要）

- **D1：权限数据归属**：权限目录与授权存储放新 addon `src/addon/access/`（域职责独立，不污染 account；符合"Addon 层目录只承载 mod.rs、module 子目录与 domain/ 的约定）。
- **D2：引导式一次性管理员（2026-09-24 修订）**：系统仍**不提供不可降权的超然账号**，应用内仍**不存在自提权路径**（组管理操作受「不得使调用者自身有效权限增大」的子集校验约束，见 `docs/contracts/AUTHZ_GRANTS.md`）；但第一个成功注册的账号由应用在**同一事务内**引导为系统管理员——它是一条普通、可降权、可停用、可删除的授权事实（内置全权组 `system_admin` 的一条成员行）。并发仲裁完全交给数据库唯一约束（`system_owner` 表的 `UNIQUE(sentinel_key)` + `CHECK`），**不做任何「用户表为空则本次注册者晋升」式的判空逻辑**。管理 Action 要求权限 `access.grants.*` / `access.groups.*`。
  - 原表述（2026-09-03，作为修订记录保留）：系统仍不提供"最终管理员"角色；初始授权由运维经 SQL/工具完成（文档化），应用不提供自提权路径。管理 Action 只要求权限 `access.grants.*`，不做任何"第一个用户自动是管理员"逻辑。
  - 修订后仍然保留的性质：无自提权路径；没有不可审计的超级身份（管理员 = 一条可查询的 `user_group` 事实）。放宽的只有「初始授权是否依赖运维人肉操作」——应用引导一次，运维 SQL 降为灾备路径。完整论证与边界见 `docs/architecture/2026-09-24-permission-groups-and-bootstrap-design.md` §3.1。
- **D3：权限目录来源**：权限目录 = 运行期从 Catalog 投影所有 Action 声明的 `.permissions(...)` 集合（单一事实来源，不另维护静态清单）。
- **D4：授权存储两层——直授 + 一层权限组（2026-09-24 标注：已按预留接口扩展出一层权限组；仍不引入组嵌套与角色继承）**：仍存"用户 ↔ 权限字符串"直授关系（`authz_grant` 表），并在其上扩展**一层**权限组聚合（`permission_group` / `permission_group_item` / `user_group`），有效权限 = 直授 ∪ 全部组成员权限的并集；Token 中仍只承载具体权限字符串（组名不进入 `roles`），角色仍用固定 `"user"`。
  - 原表述（2026-09-03，作为修订记录保留）：Phase 1 只存"用户 ↔ 权限字符串"直授关系（`authz_grant` 表），不引入角色聚合层；角色仍用固定 `"user"`。后续如需角色再扩展（留接口）。
