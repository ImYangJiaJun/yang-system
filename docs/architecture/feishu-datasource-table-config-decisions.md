...
- ~~台账 / 卡片的「加密返回」「默认语言」两列与 `DatasourceItem` 上那两个字段：`list_datasources` 已经不再投影它们，所以恒显示「—」。要真恢复得后端先加回投影（或改用字段绑定那一层的语义）。~~
  **已解决（2026-09-23，Ruling 55）**：按第二个选项——改用**字段绑定那一层**的语义。绑定投影补 `encrypt_enabled` / `default_locale`，表级那两列与卡片徽标删掉，逐字段的取值进详情页的字段绑定表。
...
- ~~详情页按**字段标识**路由（`/feishu/datasources/:sourceKey`）不变：选项端点仍是按 `source_key` 索引的，一条表级行有 N 条绑定，列表页取第一条当入口；一条绑定都没有的行点不进详情页，页面给一句说明而不是静默无响应。~~
  **已解决（2026-09-23，Ruling 53）**：详情页改按**表级主键** `id` 路由；字段绑定表（Ruling 55）就是切换器，一条绑定都没有的行也能进详情页并看到「还没有字段绑定」。
...
Ruling 53: 详情页的**身份裁决为表级主键 `id`**，且裁决由**编译器**保管。
— 事实：本页原先按 `:sourceKey` 路由，而它自己的查询还拿 `source_key` 去 `order_by`
  与 `search` 一张**没有这一列**的表级表（`validate_order_field` → 400；检索只覆盖
  `title` → 命中零行）。两处都失败 ⇒ `datasource` 恒为 null ⇒ 同步/体检/凭据三块
  一起说「查不到这条数据源」，而那条源就在库里、9 条绑定、服务端一分钟前刚拉成功。
— 落地：路由、查询、类型三处一起改；并**删掉 `DatasourceItem.sourceKey`** 这个恒为空串
  的旧身份字段（它原来的注释是「保留是为了不把存量调用点一次全改掉」——留一条已被否决
  的身份，等于给下一个消费方留一条错路）。删掉之后，任何想再用它的人会**编译不过**。
— Cost if wrong: 旧深链（`/feishu/datasources/<source_key>`）失效，页面明说「地址里没有
  有效的数据源主键」；没有对外引用，无实际代价。

Ruling 54: 详情页「取数据源」分**五态**，**失败不等于空**。
— 事实：`?.items.find(...) ?? null` 把「请求被拒」与「没有这一行」折成同一个 null，
  页面于是替用户断言了一个它并不知道的原因。区分它们不是措辞问题，是**下一步动作**
  问题（重试并看错误码 vs 回列表确认）。
— 落地：纯函数 `datasourceGap`（加载中 / 地址不合法 / 无权限 / 请求被拒 / 确认不存在），
  失败态原样显示 `status` / `code` / `request_id`；三个区块共用**一处**说明，不再各说一遍。
— 附带修正：`datasource.read` 缺失时查询是 `enabled: false`，`isPending` **永远**为真——
  不单独判这一态，页面就是一块永远转下去的骨架屏。
— Cost if wrong: 详情页多一个状态分支要维护；可接受。

Ruling 55: 「加密返回 / 默认语言」改用**字段绑定那一层**的语义（遗留缺口的第一条）。
— 落地：绑定投影补 `encrypt_enabled` / `default_locale`；表级行那两列与卡片徽标删掉；
  新增 `FieldBindingsTable`（字段绑定表），它同时是选项区的**切换器**——「像表格一样看整表」
  与「切到某个字段」本来就是同一件事的两面。
— **一度还想投影绑定的 `last_push_at`，查完撤了**：那一列**没有任何写入路径**——`pull.rs`
  只把它写进 `feishu_option`（选项行），绑定的那次 update 只写 `field_name` 与
  `snapshot_digest`。投影它就等于新造一个恒画「—」的列，正是本条要修掉的那种假话。
  逐字段的「选项数 / 最近推送」需要一个聚合查询（`GROUP BY source_key` 取 `COUNT(*)` 与
  `MAX(updated_at)`），留给下一轮——它同时供给这两列。
— 关键教训：这两个字段**没有被删，只是搬到了绑定层**。所以「扫已删字段」那类检查对它们
  **全盲**（`encrypt_enabled` 在绑定表上依然是合法字段名），只有**投影 diff**
  （后端 emit ↔ 前端 parse ↔ TS 类型 三方对齐）才看得见。这条已写进下一轮的 ③。
— Cost if wrong: 绑定投影多三个键（一行三列，同一次查询）。

Ruling 56: 结构防线第一件 = **投影契约**（`frontend/contracts/feishu-projections.json`）。
— 目标：把「后端 emit 的键集」与「前端 read 的键集」摆在一起比。这一整类 bug（前后端四次）
  的病根是「字段**没被删，只是搬到了另一层**」——`encrypt_enabled` 在绑定表上依然是合法
  字段名、`linkage_mapping` 连注释都写对了代码却还在读，所以「扫已删字段」那类检查对它**全盲**。
— 落地：一份入库的契约文件，**两端各读一次**并各自断言（Rust 用 `include_str!`，编译期读入；
  前端用 JSON import）。键分三个去处，互斥且穷尽：`emitted`（真的发出去）、
  `query_only`（查询要用但不发，例如分组键 `datasource_id`）、
  `backend_only`（发了但前端不读，**必须写明理由**——是决定，不是垃圾桶）。
— 断言：struct 序列化键 == emitted；查询选的列 == emitted ∪ query_only；
  前端 read ⊆ emitted；emitted \ read == backend_only 的键集；
  再由契约**生成**探针载荷喂真实解析器，逐键断言「真的被读到」且「解析结果没有契约之外的字段」。
— 证据（三个方向都实测会红）：前端多读一个键（**原 bug 的形状**）⇒ 前端 3 条测试红；
  契约凭空多一个 emitted 键 ⇒ Rust struct 断言红；查询少选一列 ⇒ Rust 查询列断言红。
— **它抓到的第一件真事**：`DatasourceItem.linkageMapping`——后端从不发 `linkage_mapping`，
  前端却一直在解析它、且没有任何消费方。已删。那句注释甚至已经写着「都已不在表级行上」，
  代码却还在读：**这就是「注释对、代码不对，而没有任何东西在检查」的样子。**
— Cost if wrong: 多一份要维护的契约文件 + 两端各一次断言；都属于「改列时顺手改一处」。
— 还没做：契约目前只覆盖 `list_datasources`。其余端点（`list_options`、`health_check`、
  `pull_schedule`、create/update 的响应）按**同一形状**逐个补——每补一个就是一次同样的对账。

Ruling 57: 结构防线第二件 = **字段名回灌 `TableDefinition`**（`domain/schema_anchor.rs`）。
— 为什么它能**离线**跑：DSL 的字段校验是**急切且离线**的——`select_fields` / `where_eq` /
  `where_in` / `order_by` 在**构造查询那一刻**就查表定义，不需要数据库。于是「某个站点用了
  这张表上没有的列」可以在测试里复现成 `FieldNotFound`(400) 或 `FieldPermissionDenied`(403)，
  而那正是本仓四次同类 bug 的形态。
— 为什么**扫源码**而不是维护一张清单：一百来个站点分布在十几个文件里，手写清单必然腐烂，
  而腐烂是**静默**的（漏掉一个站点＝防线失效）。扫描是穷尽的，新写的站点自动进入射程。
— **归属怎么定**（决定严格还是宽松）：**函数体唯一锚点**优先，其次语句窗口，都不行才降级。
  这一步是必需的：本仓最常见的形态是
  `let mut query = context.options().query()…;` 隔若干行才 `query.where_eq("source_key", …)`
  ——**历史上那个 bug 正是这个形状**（在表级行的 query 上筛属于绑定层的列），
  只看语句窗口会把它判成「归属不明」而放行。
— 实测：扫描 **135** 个站点，其中 **120 个严格**（列必须存在 + 能力位必须开着）、
  **15 个宽松**（只要求「三张表之一上有这一列」）。宽松档逐个钉在测试里，**不许悄悄变长**
  ——静默降级正是这一类 bug 能活下来的方式。
— **正对照**：拿一段故意写错的源码喂扫描器（表级行上 `where_eq("source_key")`、
  `order_by("status")`），两条都必须被抓到。没有它，「不报错」就分不清是代码干净
  还是扫描器瞎了——而一个瞎掉的对账器比没有更危险：它给人「已经查过」的错觉。
— **端到端证据**：在真实站点上把 `where_eq("id")` 改成 `where_eq("source_key")`，主断言
  当场报出「`delete_datasource_table.rs:112` where_eq 用了 feishu_datasource 上**不存在**的列
  `source_key` ⇒ 运行期 FieldNotFound(400)」。
— 与投影契约的分工：投影契约管「后端 emit ↔ 前端 read」（两边都手写，可以一起改错）；
  这一层管「代码里的字段名 ↔ `table_spec()` 这份唯一事实源」。
— 还没覆盖：`where_tree` 的条件树（收的是 JSON 树不是字面量）、`Record::require/optional`
  的键（record 的来源静态解析不到）、以及「同一次调用里决定**写哪些行**的 where 是否与写入
  列表在同一张表上」。三条都写在这里，别当成已经覆盖了。
