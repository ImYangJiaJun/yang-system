...
公开注册现在把“拥有该邮箱”作为创建全局 `users` 账户的前置事实。
...
启动应用，由声明式 Schema 预检并同步 `users` 的
邮箱列、唯一索引和“邮箱与验证时间同时为空或同时非空”的强制 CHECK，再滚动应用。
变更只新增可空列、唯一索引和 CHECK，因此旧账户继续可登录；新注册
...
该用例覆盖 Schema 约束、Redis 明文泄漏、冷却、已有账户抑制、错误次数、过期、SMTP
失败清理以及同一验证码并发只能成功一次。
